...
void read_board(sudoku& s)
{
    ...
    for (int r = 0; r < 9; ++r) {
        for (int c = 0; c < 9; ++c) {
            printf("Enter the element in cell (%d, %d):", r, c);
            scanf("%d", &v);
            s.b[r][c] = v;
        }
    }
...
{
    sudoku s;
    printf("Write the Input Values from the Left-Top Most cell to the Right-Bottom Most cell. Empty cell can be replaced by a 0.\n");
    read_board(s);
